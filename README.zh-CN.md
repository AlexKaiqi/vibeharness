...
先作为 agent skill 安装，不需要安装 Python package：
...
之后让你的 coding agent 使用 `$vibeharness` 来执行 checkpointed、可回放的任务恢复。

如果要运行仓库验证和 examples，再从 checkout 执行：
...
CLI 是可选增强，用于 episode scaffolding、scoring 和 reports：
...
当前版本聚焦可回放的工作流机制和轻量 skill 安装。更大的实证主张需要脱敏真实 episodes、baselines 和 transfer evaluation 支撑。
