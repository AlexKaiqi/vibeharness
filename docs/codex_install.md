...
VibeHarness supports Codex through two artifacts:

| Purpose | Repository artifact | Installed location |
| --- | --- | --- |
| Global Codex skill | `skills/vibeharness/` | `${CODEX_HOME:-$HOME/.codex}/skills/vibeharness` |
| Project instructions | `AGENTS.md` | `/path/to/project/AGENTS.md` after `vh init` |

The repository does not keep a checked-in `.codex/` directory. `install.sh`
creates the Codex skill directory in the user's local Codex home.
